"""
    sampling_sv(e, prev_h, rho, tau_squared, h0=0, P0=1)
...
- `rho::Vector`: (2) vector of the constant and autoregressive coefficient of the stochastic volatility (\$\\rho_0\$, \$\\rho_1\$)
...
"""
function sampling_sv(e, prev_h, rho, tau_squared, h0=0, P0=1)
    ...
    e_star = log.(e.^2) - (table4[omega, 3] .- c)
    H = 1
    R = reshape(table4[omega, 4], (1,1,T))
    h_hat, P_hat = kalman_filter(e_star, H, R, rho[2], tau_squared, rho[1], h0, P0)
    sampled_h = sampling_carter_kohn(h_hat, P_hat, rho[2], tau_squared, rho[1])
    return sampled_h
end
